...
**✅ PROD-02. Migrations strategy** — `prisma/migrations/` agora captura toda história (14 migrações). Migration `20260525150000_gam_library_assets` baseline pra GAM + Client.customFontUrl + MediaFormat.segment + Tenant white-label. Dev DB já com schema (via db push antigo) marcada como aplicada via `migrate resolve --applied`. **`npm start` agora roda `prisma migrate deploy && next start`** — Railway executa migrations toda inicialização (idempotente). **De agora em diante**: usar `npm run db:migrate:dev` (gera migration + aplica) ao invés de `db push`. Use `db push` apenas em prototipagem rápida e SEMPRE seguido de `migrate dev` antes de comitar.
