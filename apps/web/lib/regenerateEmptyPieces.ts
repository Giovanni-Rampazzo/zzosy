// Helper server-side: regenera pieces vazias de uma campanha a partir da
// matriz. Reusado por /api/campaigns/[id]/regenerate-empty-pieces (auth
// normal) E /api/campaigns/[id]/auto-recover (1-click URL).
//
// Logica pura — sem fetch interno, sem deps de session. Caller eh responsavel
// pela auth/tenant filter.
...
export interface RegenerateResult {
  ...
  skippedCount: number
  updates: Array<{ id: string; name: string | null; layerCount: number }>
}

export async function regenerateEmptyPiecesForCampaign(campaignId: string): Promise<RegenerateResult | { error: string; status: number }> {
  ...
  if (!kv?.data) return { error: "Campanha sem matriz", status: 400 }
  let kvData: any
  try { kvData = typeof kv.data === "string" ? JSON.parse(kv.data) : kv.data } catch {
    return { error: "KV.data malformado", status: 500 }
  }
  const matrixLayers: any[] = Array.isArray(kvData?.layers) ? kvData.layers : []
  if (matrixLayers.length === 0) return { error: "Matriz sem layers", status: 400 }
  ...
  const updates: Array<{ id: string; name: string | null; layerCount: number }> = []
