// POST /api/admin/oneshot-recover-latam?token=<HARDCODED>
//
// TEMPORARIO — single-use admin endpoint pra recuperar campanha LATAM
// (cmpmwofs600011su994nmru8g) sem autenticacao NextAuth. Token random
// hardcoded, hardcoded campaign ID, bypass tenant filter.
//
// User reportou 2026-05-27: 8 pecas vazias urgente, sem console/curl
// acessivel pelo claude. Solucao: 1 curl com token, eu execute.
//
// **REMOVER ESSE ARQUIVO** apos completar a recovery (commit subsequente).
...
const ONESHOT_TOKEN = "29706e162a288d7de5ba6d7142b461a6ddeb039a2dec4210e42a4759cf665574"
const CAMPAIGN_ID = "cmpmwofs600011su994nmru8g"

export async function POST(req: NextRequest) {
  const { searchParams } = new URL(req.url)
  if (searchParams.get("token") !== ONESHOT_TOKEN) {
    return NextResponse.json({ error: "Invalid token" }, { status: 403 })
  }
  // Sem tenant filter — token serve como auth pra ESTA campanha hardcoded.
  ...
  if (!kv?.data) return NextResponse.json({ error: "No KV" }, { status: 400 })
  let kvData: any
  try { kvData = JSON.parse(kv.data) } catch { return NextResponse.json({ error: "KV malformed" }, { status: 500 }) }
  const matrixLayers: any[] = Array.isArray(kvData?.layers) ? kvData.layers : []
  if (matrixLayers.length === 0) return NextResponse.json({ error: "Matrix empty" }, { status: 400 })
  ...
  function scaleLayerMask(mask: any, scale: number, offsetX: number, offsetY: number): any {
    ...
    if (mask.type === "vector" && mask.vector) {
      return { ...mask, vector: { ...mask.vector, posX: (mask.vector.posX ?? 0) * scale + offsetX, posY: (mask.vector.posY ?? 0) * scale + offsetY, width: (mask.vector.width ?? 0) * scale, height: (mask.vector.height ?? 0) * scale } }
    }
    if (mask.type === "raster" && mask.raster) {
      return { ...mask, raster: { ...mask.raster, posX: (mask.raster.posX ?? 0) * scale + offsetX, posY: (mask.raster.posY ?? 0) * scale + offsetY, width: (mask.raster.width ?? 0) * scale, height: (mask.raster.height ?? 0) * scale } }
    }
    return mask
  }
  ...
  for (const p of campaign.pieces) {
    ...
    const newData: any = {
      ...pData, version: 2, width: pieceW, height: pieceH,
  ...
  return NextResponse.json({
    ...
    totalPieces: campaign.pieces.length,
